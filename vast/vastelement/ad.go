...
// Each <Ad> contains either EXACTLY ONE <InLine> element or <Wrapper> element (but never both).
func (ad *Ad) Validate(version Version, checker MediaFileChecker) error {
	errors := make([]error, 0)
	...
	if ad.InLine != nil {
		errors = append(errors, ad.validateAdInline(version, checker)...)
...
func (ad *Ad) validateAdInline(version Version, checker MediaFileChecker) []error {
	errors := make([]error, 0)
	if err := ad.InLine.Validate(version, checker); err != nil {
